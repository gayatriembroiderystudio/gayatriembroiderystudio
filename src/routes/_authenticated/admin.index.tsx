...
function Dashboard() {
  ...
  const cards = [
    { label: "Bookings", value: stats.data?.bookings ?? 0, to: "/admin/bookings", icon: CalendarCheck },
    { label: "Inquiries", value: stats.data?.inquiries ?? 0, to: "/admin/inquiries", icon: MessageSquare },
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Overview of activity across your boutique.</p>
      </div>
      ...
      <section className="rounded-xl border border-border bg-card p-6 shadow-soft">
        <div className="flex items-center justify-between">
          ...
          <Link to="/admin/bookings" className="text-xs text-gold hover:text-maroon">View all →</Link>
        </div>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="text-left text-xs uppercase tracking-wider text-muted-foreground">
              <tr><th className="py-2">Name</th><th>Service</th><th>Status</th><th>Date</th></tr>
            </thead>
            <tbody>
              {(recent.data ?? []).map((r) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td><span className="rounded-full bg-gold/15 px-2 py-0.5 text-xs text-maroon-deep">{r.status}</span></td>
                  <td className="text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
              {!recent.isLoading && (recent.data?.length ?? 0) === 0 && (
                <tr><td colSpan={4} className="py-6 text-center text-muted-foreground">No bookings yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
