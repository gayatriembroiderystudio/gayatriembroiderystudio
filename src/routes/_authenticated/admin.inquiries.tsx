  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">Contact form messages from website visitors.</p>
      </div>
      <div className="space-y-4">
        ...
        {data?.map((m) => (
          <div key={m.id} className="rounded-xl border border-border bg-card p-5 shadow-soft">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h3 className="font-display text-lg text-maroon-deep">{m.subject || "(no subject)"}</h3>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => remove(m.id)} className="rounded p-2 text-destructive hover:bg-destructive/10">
        ...
        {!isLoading && (data?.length ?? 0) === 0 && (
          <p className="rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground">No inquiries yet.</p>
        )}
      </div>
    </div>
  );
