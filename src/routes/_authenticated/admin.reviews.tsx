    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Approve customer reviews to display publicly.
        </p>
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        ...
        {data?.map((r) => (
          <div key={r.id} className="rounded-xl border border-border bg-card p-5 shadow-soft">
            <div className="flex items-start justify-between gap-2">
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-xs ${r.approved ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
              >
              ...
              <div className="flex gap-2">
                {r.approved ? (
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full bg-maroon px-3 py-1 text-xs text-cream"
                  >
        ...
        {!isLoading && (data?.length ?? 0) === 0 && (
          <p className="rounded-xl border border-dashed border-border p-8 text-center text-muted-foreground md:col-span-2">
            No reviews yet.
          </p>
        )}
      </div>
